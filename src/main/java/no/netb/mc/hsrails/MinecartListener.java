...
public class MinecartListener implements Listener {
    ...
    private static final double MAX_SPEED_METERS_PER_TICK = 32.0 / 20.0; // 32 m/s divided by 20 ticks per second
    ...
    public void onVehicleMove(VehicleMoveEvent event) {
        ...
        if (event.getVehicle() instanceof Minecart) {
            ...
            Block blockBelow = cartsWorld.getBlockAt(cartLocation.clone().add(0, -1, 0));
            ...
            double maxSpeed = DEFAULT_SPEED_METERS_PER_TICK * HsRails.getConfiguration().getSpeedMultiplier();
            if (speed > maxSpeed) {
                Vector clampedVelocity = velocity.normalize().multiply(maxSpeed);
                cart.setVelocity(clampedVelocity);
            }
        }
    }
}
